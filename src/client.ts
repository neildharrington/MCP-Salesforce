...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['SFDC_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['SFDC_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Sfdc API.
 */
export class Sfdc {
  apiKey: string;
   ...
   * @param {string} [opts.baseURL=process.env['SFDC_BASE_URL'] ?? https://petstore3.swagger.io/api/v3] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('SFDC_BASE_URL'),
    apiKey = readEnv('PETSTORE_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.SfdcError(
        "The PETSTORE_API_KEY environment variable is missing or empty; either provide it, or instantiate the Sfdc client with an apiKey option, like new Sfdc({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? Sfdc.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('SFDC_LOG'), "process.env['SFDC_LOG']", this) ??
      defaultLogLevel;
  ...
  static SfdcError = Errors.SfdcError;
...
Sfdc.PetResource = PetResource;
Sfdc.Store = Store;
Sfdc.UserResource = UserResource;
export declare namespace Sfdc {
